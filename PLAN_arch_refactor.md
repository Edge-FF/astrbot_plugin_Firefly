...
| 项 | 提交 | 内容 |
|---|---|---|
| P2-1 | `d033823` | 新建 `core/config.py`（`ShellConfig` + `ProactiveConfig`，241 行） |
| P2-2 | `966f5be` | 新建 `core/records.py`（140 行）与 `adapter/dto.py`（70 行）；`to_api_dict` 移出领域层 |
| P2-3 | `fd7999f` | `parsers` 补齐 `coerce_float` / `coerce_bool`；两份重复的嵌套 `_int/_float/_bool` 删除；配置脏值开始产生告警 |
| P2-4 | —（无 diff） | import 迁移已在各子项内完成，本项转为自检步骤，见下 |

**结果指标**

| 指标 | P1 结束 | P2 结束 |
|---|---|---|
| `core/models.py` 行数 | 704 | **308**（目标 ≤320 ✅） |
| 测试数 | 198 | **210**（+12：`tests/test_config.py`） |
| `tests/` lint 债 / 格式债 | 11 / 8 文件 | 11 / 8 文件（无新增） |

**P2 的行为等价证据（强于单纯跑测试）**

1. **逐字节搬运证明**：`core/config.py` 的正文与旧 `models.py` 第 317–542 行**逐字节相同**（脚本比对 `old_region == body` 为 True）。
2. **DTO 输出黄金比对**：重构前先固化 `to_api_dict()` 的 JSON 输出（含全部字段与 `round`/切片边界），重构后由 `adapter.dto` 的函数产出同一 JSON，比对结果**逐字节一致**。
3. **锚点测试不变**：`tests/test_behavior_anchor.py` 的冻结注入文本与 `SessionState` 断言未做任何修改即通过——P2 未触碰注入行为。

**P2-4 自检结果**

- `ruff check --select F401,F811,F821 core adapter main.py tests`：仅 1 处告警，为 `tests/test_loader.py` 的**既有债**（§2.3 已记录），非 P2 引入。
- 自动扫描「仍从 `core.models` 导入已迁出类型」：**无**。
- 架构护栏 4 条全通过；锚点 3 条全通过。
- 运行期接线核对：`main.py` 两个 getter 均收集告警并调用 `_log_config_warnings`；去重逻辑实测 —— 同来源同批告警只记一次、不同来源互不干扰、问题消失后不重复记录。

#### P2 对计划的两处偏离

| 项 | 计划 | 实际 | 原因 |
|---|---|---|---|
| P2-4 | 由 P2-4 统一迁移 import | 各子项内随改随迁，P2-4 变为自检步骤 | 若把 import 迁移推迟到 P2-4，则 P2-1～P2-3 三个 commit 处于「导入即失败」状态，违背 §6.2「每个 commit 可独立回滚且保持可用」 |
| P2-3 | 仅让 `from_dict` 支持收集告警 | 同时把告警接入 `main.py` 的 getter 并做去重 | 告警若无人消费等于没有可观测性（与 §3.3.1「死配置」同类问题）；getter 每轮注入都会被调用，故按来源去重避免刷屏 |

> **P2-3 是行为变更**（脏配置从静默回退变为回退 + 告警），已在 commit message 中标注，
> 且不改变任何回退值本身——`test_config.py` 同时锁定「回退值」与「告警文本」两侧。
...
- [x] **P2-1** `core/config.py`；`d033823`
- [x] **P2-2** `core/records.py` + `adapter/dto.py`；`966f5be`
